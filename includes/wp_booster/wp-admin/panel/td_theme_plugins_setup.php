...
class td_theme_plugins_setup {
	...
	private function _get_plugins() {
		...
		foreach ( $instance->plugins as $slug => $plugin ) {
			...
			if ( class_exists( $plugin['td_class'], false ) && false === $instance->does_plugin_have_update( $slug ) && false === $this->theme_plugin_has_update( $slug ) ) {
                continue;
			} else {
	...
	public function theme_deactivated_plugin( $plugin ) {
		...
		if ( isset( $theme_deactivated_plugin_array[$plugin['slug']] ) ) {
			...
			if ( false === $this->theme_plugin_has_update( $plugin['slug'] ) && td_util::is_active( $plugin ) ) {
				...
				return $theme_deactivated_plugin_array[$plugin['slug']];
			}
		}
